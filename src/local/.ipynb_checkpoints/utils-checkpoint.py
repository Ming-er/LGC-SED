...
def batched_decode_preds(
    strong_preds, filenames, encoder, thresholds=[0.5], median_filter=[3,28,7, 4,7,22,48,19,10, 50], pad_indx=None,
):
    ...
    for j in range(strong_preds.shape[0]):  # over batches
        for c_th in thresholds:
            ...
            pred = pred > c_th
            pred_lst = []
            for lb in range(pred.shape[1]):
                cls_pred = pred[:, lb]
                cls_pred = scipy.ndimage.filters.median_filter(cls_pred, median_filter[lb])
                pred_lst.append(cls_pred.reshape(-1, 1))
            pred = np.concatenate(pred_lst, axis=1)
