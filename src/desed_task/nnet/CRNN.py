...
class CRNN(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.cnn_proj_fc = nn.Linear(n_RNN_cell * 2, n_RNN_cell)
        self.cnn_proj_bn = nn.BatchNorm1d(nb_in, eps=0.001, momentum=0.99)
        self.cnn_relu = nn.LeakyReLU(inplace=False, negative_slope=0.2)
    ...
    def forward(self, x, pad_mask=None, embeddings=None, proj=False, refine=False, prototype_vec=None):
        ...
        # projector: conv + relu + norm
        if proj:
            feat = self.cnn_proj_fc(x)
            feat = self.cnn_relu(feat)   
            embed_dim = feat.size(-1)
            feat = feat.reshape(-1, embed_dim)
        ...
        if proj:
            return strong.transpose(1, 2), weak, feat
        else:
